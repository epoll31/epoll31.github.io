  ...
  return (
    <div className="grid grid-flow-row grid-cols-1 lg:grid-cols-2 w-full p-5 justify-evenly gap-2" id="courses">
      <div className="col-span-full flex flex-row sm:grid grid-cols-1 grid-rows-1 grid-flow-row w-full">
        <h1 className="flex-grow row-span-full col-start-1 text-2xl text-center text-nowrap" >Courses Taken</h1>
      ...
      </div>
      {
        courses.length == 0 ? 
          ...
          courses.map((c: Course) => <CourseCard course={c} activeTags={activeTags} className="" key={c.name}/>)
      }
    </div>
  );
}